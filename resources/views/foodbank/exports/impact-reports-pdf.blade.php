...
<head>
    ...
    <title>Impact Report - {{ $foodbankName }}</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <div class="header">
        <h1>Impact Report</h1>
        <div class="subtitle">{{ $foodbankName }}</div>
            ...
            <tr>
                <td style="width: 50%; padding: 10px;">
                    <div class="summary-card">
                        ...
                        <div class="value">{{ number_format($foodRequests) }}</div>
                    </div>
                </td>
                <td style="width: 50%; padding: 10px;">
                    <div class="summary-card">
                        <div class="label">Food Received</div>
                        <div class="value">{{ number_format($foodReceived) }}</div>
                    </div>
                </td>
            </tr>
        </table>
    ...
    <div class="summary-section">
        <div class="section-title">Top Establishment Contributors</div>
        <table class="data-table">
            <thead>
                <tr>
                    <th>Rank</th>
            ...
            <tbody>
                @if(!empty($topContributorsData))
                    @foreach($topContributorsData as $contributor)
                        <tr>
                            <td>{{ $contributor['rank'] }}</td>
                            <td>{{ $contributor['establishment_name'] }}</td>
                            <td>{{ number_format($contributor['completed_requests']) }}</td>
                            <td>{{ $contributor['percentage'] }}%</td>
                        </tr>
                    @endforeach
                @else
                    <tr>
                        <td colspan="4" class="no-data">No contributor data available</td>
                    </tr>
                @endif
            </tbody>
        </table>
