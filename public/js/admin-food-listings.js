...
function updateStatus(id, status) {
    ...
    const actionText = status === 'active' 
        ? 'enable this food item and make it visible to customers' 
        : 'disable this food item and hide it from all customer-facing pages';
    ...
    const buttons = document.querySelectorAll(`button[onclick*="updateStatus(${id}"]`);
    let button = null;
    if (buttons.length > 0) {
        button = Array.from(buttons).find(btn => btn.onclick.toString().includes(`'${status}'`));
    }
